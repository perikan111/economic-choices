...
佐藤美咲・山田浩一の2名に立ち絵を実装済み。どちらも `defaultExpression` は `neutral`、
表情は 5 種で共通（`neutral` / `concern` / `frustrated` / `relieved` / `serious`）。
この表示 metadata は台詞・分岐・効果・セーブ状態へ影響しない。

| 人物 | 画像 ID | キャンバス | 画像パス |
| --- | --- | --- | --- |
| 佐藤美咲 | `misaki.<expression>` | **384×1040** | `bread-price/characters/misaki/misaki_<expression>.png` |
| 山田浩一 | `yamada.<expression>` | **1024×1536** | `bread-price/characters/yamada/yamada_<expression>.png` |

**キャンバス寸法は人物ごとに異なってよい。** 山田は体格が横に広く、原画のアスペクト比が美咲と違う。
画像を統一寸法へリサイズ・変形・トリミングすることはしない。

表示は **高さ基準**（`.character-portrait-slot` の `height` に対して `height: 100%` / `width: auto`）で行い、
アスペクト比は `object-fit: contain` で保持する。縦の占有率が美咲 93.8% / 山田 99.0% のため、
同じスロット高さで表示したときの見かけの身長比は **約 1.05**（山田がやや高い）に収まる。

> **立ち絵カラム幅の下限。** 横に広い人物が高さいっぱいに表示されるには、
> カラム幅 ≧ スロット高 × 画像アスペクト比 が必要になる。
> 山田（1024:1536）でスロット高 540px のとき 360px。
